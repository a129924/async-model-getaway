...
3. `ResponseCacheKeyFactory` 在建構時持有 `FeatureHasher`，並暴露 `build(...)`，且該方法必須顯式接收 `namespace`、顯式 `payload_hasher`、顯式 `payload` 與顯式 `features: Mapping[str, str]`。
4. `ResponseCacheKeyFactory.build(...)` 必須將 payload hashing 委派給顯式傳入的 `ModelPayloadHasher`，並將 feature hashing 委派給持有的 `FeatureHasher`。
...
### Scenario 1: 由顯式 hash owners 建立 keyed cache identity

- **Given**: 一個以 `FeatureHasher` 建構的 `ResponseCacheKeyFactory`
- **And**: 顯式的 `ModelPayloadHasher`、顯式 `namespace`、顯式 payload 與顯式 `features: Mapping[str, str]`
- **When**: 呼叫 `build(...)`
- **Then**: payload 會透過顯式的 `ModelPayloadHasher` 進行 hashing
- **And**: features 會透過持有的 `FeatureHasher` 進行 hashing
- **And**: 回傳的 `ResponseCacheKey` 只包含提供的 `namespace` 與兩個 hashing 結果

### Scenario 2: 不支援的 payload 會 fail closed

- **Given**: 一個會被既有 payload-hash owner 拒絕的 payload
- **When**: 呼叫 `build(...)`
- **Then**: 既有 payload-hash failure 會原樣往外傳播
...
- 既有 `ModelPayloadHasher` semantics 仍是唯一權威：dict-key canonicalization 仍屬於 payload hasher，而不是新的 factory。
