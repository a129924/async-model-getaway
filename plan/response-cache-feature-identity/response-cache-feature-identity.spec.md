...
2. It validates every runtime mapping key and value as `str`; for every accepted built-in string or `str` subclass, it obtains the identity material with `str.__str__(key)` and `str.__str__(value)` without calling an override of `__str__`; then it sorts every resulting base-string pair lexicographically as a complete `(base_key, base_value)` tuple.
...
4. Equivalent `Mapping[str, str]` collections produce the same digest irrespective of insertion order, `str` subclass comparison behavior, or overridden `__str__`; a changed base key or value remains a distinct identity even if subclass `__str__` overrides would produce the same text. A mapping-like source that emits equal base keys with different base values remains valid and yields one deterministic digest irrespective of emitted order; it is neither rejected nor deduplicated.
...
### Scenario 5: Equal base keys with distinct values use complete-pair ordering

- **Given**: two mapping-like inputs whose `.items()` emit `str` subclass keys that canonicalize to the same base key and values with distinct base-string content, but emit the two pairs in opposite orders.
- **When**: a caller hashes each input directly and supplies each to `ResponseCacheKeyFactory(CanonicalFeatureHasher())`.
- **Then**: all results use the same sorted pair-list ordered lexicographically by complete `(base_key, base_value)` tuples and therefore have the same digest. Neither input raises a duplicate-key error or loses either pair through deduplication.

## Error / Edge Cases

- An empty mapping is valid and must preserve the frozen `[]` golden digest.
- Sorting happens only after runtime type validation and base-string material extraction, and compares complete `(base_key, base_value)` tuples, so mixed key types cannot produce an incidental sorting exception, subclass comparisons cannot alter ordering, and equal base keys with different values remain insertion-order independent.
...
- Tests assert direct-hasher and factory-integration behavior for duplicate base keys as well as factory-propagation behavior, including `TypeError.__cause__` being a `UnicodeEncodeError`; tests import `_canonical_feature_hasher` directly and dynamic module loading is prohibited.
